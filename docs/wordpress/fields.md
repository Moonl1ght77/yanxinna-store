...
| 字段 | 类型 | 发布所需 |
| --- | --- | --- |
| `colors[].names.{locale}` | 五语 Text | 是，五种语言都必填 |
...
| 字段 | 类型 | 规则 |
| --- | --- | --- |
| `parameters[].labels.{locale}` | 五语 Text | 添加该参数时五语都必填 |
| `parameters[].values.{locale}` | 五语 Text | 添加该参数时五语都必填 |
...
| `attachments[].labels.{locale}` | 五语 Text | 添加附件时五语都必填 |

## 每种语言的字段

以下字段位于 `translations.{locale}`，五种语言使用同一结构：
...
这条规则用于防止半成品进入前台。正确流程是“导入/创建草稿 → 补齐字段 → 预检 → 发布”。
